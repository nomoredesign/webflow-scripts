...
async function saveResults(formData: FormData) {
  ...
  for (let rn = 1; rn <= 3; rn++) {
    ...
    for (let gn = 1; gn <= 3; gn++) {
      ...
      // Stop after clinching (best of 3)
    ...
    // Delete existing games then re-insert
  ...
  // Mark fixture complete if all 3 rubbers have a winner
  ...
  revalidatePath(`/dashboard/fixtures/${fixtureId}`)
  revalidatePath('/dashboard/fixtures')
}

export default async function FixtureResultPage({ params }: { params: Promise<{ fixtureId: string }> }) {
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">
          ...
          {fixture.scheduled_date && ` · ${new Date(fixture.scheduled_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}`}
        </p>
